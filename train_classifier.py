...
LEARNING_RATE = 0.001
EPOCHS = 10 
SAMPLES_PER_CLASS = 10000 # Total 40,000 sentences
...
def prepare_data():
    ...
    # 2. YELP DATASET (Negative / Positive)
    ...
    # Combine into tuples: (Text, Class_ID)
    dataset_tuples = []
    dataset_tuples.extend([(text, 0) for text in toxic_texts])   # Class 0: Toxic
    dataset_tuples.extend([(text, 1) for text in polite_texts])  # Class 1: Polite
    dataset_tuples.extend([(text, 2) for text in negative_texts])# Class 2: Negative
    dataset_tuples.extend([(text, 3) for text in positive_texts])# Class 3: Positive
